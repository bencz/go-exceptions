...
func TestAllHelperFunctions(t *testing.T) {
...
func TestInnerExceptionMethods(t *testing.T) {
...
func TestPanicRecovery(t *testing.T) {
	t.Run("Try should recover from panic", func(t *testing.T) {
		var caught bool
		...
		Try(func() {
			...
			caught = true
			panicMessage = ex.Error()
		})

		if !caught {
			t.Error("Try should catch panic as exception")
		}
		if !strings.Contains(panicMessage, "Test panic message") {
			t.Errorf("Panic message should be preserved, got: %s", panicMessage)
		}
	})

	t.Run("Try should handle different panic types", func(t *testing.T) {
